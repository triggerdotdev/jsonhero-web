...
function validateVersion(uuid$1, versionNumber) {
    return uuid.validate(uuid$1) && uuid.version(uuid$1) === versionNumber;
}
...
function inferJson(value) {
    ...
    try {
        const parsedValue = JSON5__default["default"].parse(value);
        if (typeof parsedValue === "object") {
            return {
                name: "json",
                variant: "json5",
...
function inferJWT(value) {
    try {
        const token = jwtDecode__default["default"](value);
        if (token) {
            return {
                name: "jwt",
            };
